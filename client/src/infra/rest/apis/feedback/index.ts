...
export const submitFeedback = async (payload: SubmitFeedbackPayload) => {
    ...
    return post<
        FormData,
        ApiResponse<{ feedback: FeedbackItem }>
    >('/api/feedback/submit', true, formData, false);
};

export const getUserFeedback = async (params?: { limit?: number; skip?: number }) => {
    const queryString = params ? `?${new URLSearchParams(params as any).toString()}` : '';
    return get<undefined, ApiResponse<{ feedback: FeedbackItem[]; total: number; hasMore: boolean }>>(
        `/api/feedback/user${queryString}`,
        true
    );
};
