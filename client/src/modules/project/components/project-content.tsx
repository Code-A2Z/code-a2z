...
const QuoteBlock = ({ data }: { data?: EJData }) => {
  ...
  return (
    ...
    >
      ...
      {caption ? <Typography variant="body2" color="purple">{caption}</Typography> : null}
    </Box>
  );
};
...
const ListBlock = ({ data }: { data?: EJData }) => {
  ...
  return (
  <Box component={Tag as React.ElementType} pl={3} sx={{ listStyleType: styleType }}>
      {items.map((item, i) => (
        <li key={i}>
          <Typography dangerouslySetInnerHTML={{ __html: item }} variant="body1" />
        </li>
      ))}
    </Box>
  );
...
const CodeBlock = ({ data }: { data?: EJData }) => {
  ...
  return (
    ...
    >
      {language ? (
        <Typography
          sx={{ position: "absolute", top: 8, left: 8, fontSize: 12, opacity: 0.7 }}
        >
          {language}
        </Typography>
      ) : null}
      <Button size="small" onClick={handleCopy} variant="outlined" sx={{ position: "absolute", top: 8, right: 8 }}>
        {copied ? "Copied" : "Copy"}
      </Button>
      <Typography component="pre" sx={{ fontFamily: "monospace", mt: 4, whiteSpace: "pre-wrap" }}>
        {codeText}
      </Typography>
    </Box>
  );
